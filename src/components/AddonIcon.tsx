...
const AddonIcon: React.FC<AddonIconProps> = ({ name, className }) => {
  ...
  useEffect(() => {
    fetch('/data/addons.json')
      ...
      .then((data: AddonData) => {
        const addonKey = Object.keys(data).find((key) => key === name)
        if (addonKey && data[addonKey].image) {
          setImagePath(`${data[addonKey].image}`)
          setRarity(data[addonKey].rarity)
          setAddonName(data[addonKey].name)
  ...
  if (!imagePath) {
    return <div className={`addon-icon ${className || ''}`}>Loading...</div>
  }

  return (
    <div className={`addon-icon ${className || ''} relative`} onMouseEnter={() => setShowTooltip(true)} onMouseLeave={() => setShowTooltip(false)} onMouseMove={handleMouseMove}>
      <Tooltip name={addonName} description={description} isVisible={showTooltip} position={tooltipPosition} />
      <div className={`border-2 ${getBorderColor()} p-1`}>
        <img src={imagePath} alt={`${name} Addon`} className="w-full h-full object-contain" />
      </div>
    </div>
  )
}
